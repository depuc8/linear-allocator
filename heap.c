...
static inline uintptr_t align_forward_uintptr(uintptr_t ptr, size_t alignment){
...
void* la_alloc(LinearAllocator *la, size_t size, size_t alignment){//returns a pointer for the process 
	...
	uintptr_t aligned = align_forward_uintptr(cur_ptr,alignment);//returns the address of the aligned pointer
...
size_t la_capacity(const LinearAllocator* la) { return la->capacity; }
size_t la_offset(const LinearAllocator* la) { return la->offset; }
